...
# 운동 추천 코드
def ex_get(db, user_id: int, weather: str = "Clear", temperature: float = 20):
    ...
    if weather in ["Clear", "Clouds"] and 15 <= temperature <= 30:
        space = ["0", "1"]
    else:
        space = ["0"]
    ...
    # 추천할 운동, 영상 1개씩 고르기
    prenum = random.randint(0, len(pre_ex_rec) - 1)
    daily_pre_ex_recommend = pre_ex_rec[prenum].rec_ex
    daily_pre_ex_video = pre_ex_rec[prenum].rec_video_url
    mainnum = random.randint(0, len(main_ex_rec) - 1)
    daily_main_ex_recommend = main_ex_rec[mainnum].rec_ex
    daily_main_ex_video = main_ex_rec[mainnum].rec_video_url
    endnum = random.randint(0, len(end_ex_rec) - 1)
    daily_end_ex_recommend = end_ex_rec[endnum].rec_ex
    daily_end_ex_video = end_ex_rec[endnum].rec_video_url
